...
#         "D": "D — facet pipeline with original-image Stage 2",
#     }
...
#         [
#             "",
#             "Choose A, B, or D. After you choose, I can either browse matching products for that version or iterate on it further.",
#         ]
#     )
...
#     if errors:
#         return (
#             "I tried to generate image design options, but the image-iteration step failed. "
#             "The main issue was:\n"
#             f"{errors[0]}\n\n"
...
#             "A/B/D image design options were generated this turn, so the user "
...
#             "what_we_inferred": (
#                 "The system generated design options A, B, and D and is waiting "
...
#         "confidence_score": 20,
#         "confidence_reasoning": (
#             "The user requested image iteration, but the image-generation step failed."
#         ),
...
#         "partial_understanding": {
#             "what_user_said": "The user provided or modified an image.",
#             "what_we_inferred": "Image iteration was attempted but failed.",
#         },
#     }
...
# - D = facet pipeline with original-image Stage 2
...
# - response_to_user should say that options A, B, and D were generated.
# - Ask the user to choose A, B, or D.
# - Mention:
#   A = naive preservation baseline
#   B = vision-described baseline
#   D = facet pipeline with original-image Stage 2
...
# - Do NOT create browser_query.
# - Return FORMAT B asking the user to choose A, B, or D.
...
#   "task_type": "needs_clarification",
#   "confidence_score": 35,
#   "confidence_reasoning": "The user selected option D but did not yet ask to browse products or iterate further.",
#   "response_to_user": "Great — I’ll use option D as the design reference. Do you want me to browse matching products for this version, or keep iterating on the image?",
#   "input_modalities_used": ["text", "image"],
#   "clarifying_questions": [],
#   "partial_understanding": {
#     "what_user_said": "The user selected option D.",
#     "what_we_inferred": "Option D should be treated as the active design reference."
#   }
# }

# User: "choose D and browse matching products under $1000"
...
#         # A/B/D image options were generated. Ask user to pick one.
...
#         # User asked for image iteration but A/B/D failed.
#         # ------------------------------------------------------------
...
##Currently, the PlannerAgent is disabled because the inner LLM planner is not yet fully reliable.
...
#     current_iter = _current_image_iteration_output(state)

#     return (
#         current_iter.get("requested") is True
#         and current_iter.get("mode") == "reliable_edit"
#         and current_iter.get("ok") is True
#         and current_iter.get("pending_selection") is False
#     )
...
#     if verified:
#         line = f"Done — I applied the {op} edit and verified it."
#     else:
#         line = (
#             f"I generated an updated image for the {op} edit, but the verifier did not fully confirm the change. "
...
#         "confidence_score": 35,
#         "confidence_reasoning": (
#             "A verified/best-effort image edit was produced this turn. The pipeline should stop "
...
# # Verified Image Edit Handling

# If current_turn_image_iteration_output.mode is "reliable_edit" and ok is true:
# - Do NOT create browser_query in the same turn.
# - Return FORMAT B.
# - Tell the user the edited image is now the active design reference.
...
#         # A reliable Imagen/Gemini edit completed. Stop so user can inspect.
#         # ------------------------------------------------------------
...
def _forced_reliable_edit_completed_plan(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "confidence_score": 35,
        "confidence_reasoning": (
            "A Gemini-native verified/best-effort visual edit was produced this turn. The pipeline should stop "
            "so the user can inspect it before Browserbase product retrieval runs."
        ),
        "response_to_user": _render_reliable_edit_message(iteration_output),
        "input_modalities_used": ["text", "image"],
...
If current_turn_image_iteration_output.mode is "reliable_edit" and ok is true, the edit came from Gemini image editing:
- Do NOT create browser_query in the same turn.
- Return FORMAT B.
