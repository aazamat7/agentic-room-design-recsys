...
- image_ops.py            -> Gemini image operation registry
...
- anthropic.AnthropicVertex for optional/required edit verification only
...
PROJECT_ID = (
    os.getenv("IMAGE_ITERATION_PROJECT_ID")
...
# Keep locations explicit and separate. Gemini image/text/embedding clients are
# created with explicit locations; do not rely on process-wide GOOGLE_CLOUD_LOCATION.
GEMINI_IMAGE_LOCATION = os.getenv("GEMINI_IMAGE_LOCATION", "global")
GEMINI_TEXT_LOCATION = os.getenv("GEMINI_TEXT_LOCATION", GEMINI_IMAGE_LOCATION)
EMBED_LOCATION = (
...
# Legacy Imagen variables are kept only so older callers/env files do not break.
# They are not used for edit execution in this Gemini-native build.
IMAGEN_LOCATION = os.getenv("IMAGEN_LOCATION", "disabled")
IMAGEN_GEN_MODEL = os.getenv("IMAGEN_GEN_MODEL", "disabled")
IMAGEN_MODEL = os.getenv("IMAGEN_MODEL", os.getenv("IMAGEN_EDIT_MODEL", "disabled"))

# Do not set GOOGLE_CLOUD_LOCATION at import time. Gemini clients below use
# explicit per-client locations and do not need process-wide location mutation.
LOCATION = os.getenv("GOOGLE_CLOUD_TEXT_LOCATION", GEMINI_TEXT_LOCATION)
os.environ.setdefault("GEMINI_IMAGE_LOCATION", GEMINI_IMAGE_LOCATION)
os.environ.setdefault("GOOGLE_CLOUD_EMBED_LOCATION", EMBED_LOCATION)
...
MAX_CONCURRENT_VARIANT_PIPELINES = int(os.getenv("MAX_CONCURRENT_VARIANT_PIPELINES", "3"))
MAX_CONCURRENT_E_BRANCHES = int(os.getenv("MAX_CONCURRENT_E_BRANCHES", "2"))
...
RELIABLE_IMAGE_EDIT_TRIES = int(os.getenv("RELIABLE_IMAGE_EDIT_TRIES", "2"))
...
SCENE_EDIT_BEST_OF_N = int(os.getenv("SCENE_EDIT_BEST_OF_N", "2"))
GEMINI_EDIT_ANALYSIS_ENABLED = os.getenv("GEMINI_EDIT_ANALYSIS_ENABLED", "true").strip().lower() in {"1", "true", "yes", "y"}
...
_text_genai_client = None
_embedding_client = None
_ANCHOR_CACHE: Optional[Dict[str, Dict[str, Any]]] = None


def _make_genai_client(location: str):
    project_id = _require_project_id()
    from google import genai
    return genai.Client(vertexai=True, project=project_id, location=location)


def _get_image_genai_client():
    global _image_genai_client
    if _image_genai_client is None:
        _image_genai_client = _make_genai_client(GEMINI_IMAGE_LOCATION)
    return _image_genai_client


def _get_text_genai_client():
    global _text_genai_client
    if _text_genai_client is None:
        _text_genai_client = _make_genai_client(GEMINI_TEXT_LOCATION)
    return _text_genai_client


def _get_embedding_client():
    global _embedding_client
    if _embedding_client is None:
        _embedding_client = _make_genai_client(EMBED_LOCATION)
...
def generate_image_from_prompt(
    ...
    """
    client = _get_image_genai_client()
    from google.genai import types
...
def generate_style_fanout_branches(
    ...
    max_workers = max(1, min(MAX_CONCURRENT_E_BRANCHES, len(styles)))
...
def _branch_alignment_score(branch: Dict[str, Any], user_intent: str, scored_facets: Dict[str, List[Dict[str, Any]]]) -> float:
    """Cheap non-Claude branch selector using Gemini Embedding 2.

    The goal is to avoid generating random fan-out branches and then blindly taking
    branch 0. We compare each generated branch image against the desired style/color/
    material target text, while adding small preservation/change terms.
    """
    try:
        target_text = (
            f"Interior design target. User intent: {user_intent}. "
            f"Style: {branch.get('style')}. "
            f"Colors: {', '.join(top_labels(scored_facets, 'color', COLOR_TOP_N))}. "
            f"Materials: {', '.join(top_labels(scored_facets, 'material', MATERIAL_TOP_N))}."
        )
        target_vec = embed_text(target_text)
        branch_vec = embed_multimodal_prompt_image(GLOBAL_PROMPT, branch["image"], user_intent)
        return cosine(target_vec, branch_vec)
    except Exception as exc:  # noqa: BLE001
        print(f"[pipeline E] branch embedding score failed for {branch.get('style')}: {exc}")
        return 0.0


def select_best_style_branch(
    original_image: Image.Image,
    branches: List[Dict[str, Any]],
    scored_facets: Dict[str, List[Dict[str, Any]]],
    user_intent: str,
) -> Dict[str, Any]:
    """Select E branch without Claude.

    Score combines:
      - embedding alignment to the style/color/material target
      - architecture preservation proxy (not too much pixel drift)
      - non-identity proxy (some meaningful change must happen)
    """
    if not branches:
        raise RuntimeError("No branches available for selection.")

    scored: List[Tuple[float, Dict[str, Any]]] = []
    for branch in branches:
        align = _branch_alignment_score(branch, user_intent, scored_facets)
        mad = _mean_abs_diff(original_image, branch["image"])
        preservation = max(0.0, 1.0 - min(mad / 45.0, 1.0))
        change = min(mad / 18.0, 1.0)
        score = 0.65 * align + 0.20 * preservation + 0.15 * change
        branch["selection_score"] = float(score)
        branch["selection_components"] = {
            "embedding_alignment": float(align),
            "mean_abs_diff": float(mad),
            "preservation_proxy": float(preservation),
            "change_proxy": float(change),
        }
...
def fuse_color_then_material(
    ...
    if original_image is not None:
        ...
        color_prompt += (
            "\n\nYou are given two reference images. Use the FIRST image as the exact room/architecture "
            "anchor. Use the SECOND image only as the selected style branch/design direction. "
            "Apply the color palette while preserving the first image's room shell."
        )
        color_img = generate_image_from_prompt(
            color_prompt,
            [original_image, selected_branch["image"]],
            aspect_ratio="4:3",
        )

        material_prompt = build_material_fusion_prompt(selected_style, selected_colors, selected_materials, user_intent)
        material_prompt += (
            "\n\nYou are given two reference images. Use the FIRST image as the architecture anchor. "
            "Use the SECOND image as the current color-fused design. Apply material finishes while "
            "preserving the first image's structure and camera perspective."
        )
        material_img = generate_image_from_prompt(
            material_prompt,
            [original_image, color_img],
            aspect_ratio="4:3",
        )
        return material_img
...
def run_full_facet_pipeline(
    image: Image.Image,
    user_intent: str,
    stage2_input: str = "original_plus_branch",
    sampling_strategy: str = "top1_plus_random",
    ...
    selected_branch = select_best_style_branch(image, branches, scored_facets, user_intent)

    if stage2_input in {"original_plus_branch", "original"}:
        final_image = fuse_color_then_material(selected_branch, scored_facets, user_intent, original_image=image)
    elif stage2_input == "branch":
        final_image = fuse_color_then_material(selected_branch, scored_facets, user_intent, original_image=None)
    else:
...
def pipeline_e_facet_diverse_fanout(image: Image.Image, intent: str) -> Image.Image:
    result = run_full_facet_pipeline(
        image,
        intent,
        stage2_input="original_plus_branch",
...
# Gemini-only editing backend. We keep some legacy Imagen-named helpers so the
# rest of the module does not need to change, but all edits are executed with
# gemini-3.1-flash-image (or the GEMINI_IMAGE_MODEL override).
_imagen_model = None  # legacy placeholder; no longer used for editing


def _get_imagen():
    raise RuntimeError(
        "Imagen editing is disabled in this Gemini-only build. Use gemini-3.1-flash-image for all edits."
    )


def _mk_ref(cls, base_img, reference_id, **extra):
    raise RuntimeError("Semantic-mask Imagen references are disabled in this Gemini-only build.")


def _pil_to_vimage(img: Image.Image):
    raise RuntimeError("Vertex Imagen conversion is disabled in this Gemini-only build.")
...
def _imagen_edit(
    ...
    # Backward-compatible shim: all object edits are now prompt-based Gemini edits.
    del classes, edit_mode, dilation
    return _gemini(image, prompt)



# ---------------------------------------------------------------------
# Gemini-native targeted editing
# ---------------------------------------------------------------------
# Imagen semantic masks used to localize a class id. Gemini image editing is
# different: it is instruction/reference-image based. So each targeted object
# edit first builds a natural-language target descriptor from the actual image,
# then sends a precise edit brief with preservation and negative constraints.

GEMINI_OBJECT_OPS = {"remove", "recolor", "swap", "restyle_object", "change_material"}


def _target_analysis_prompt(target: str, operation: str, edit_request: str) -> str:
    return f"""
You are preparing a TARGETED image edit for Gemini image editing.
Analyze the input room image and identify the exact object to edit.

Target object label from parser: {target}
Operation: {operation}
...
{{
  "target_found": true,
  "target_descriptor": "one sentence locating the object visually: position, color/material, nearby objects, size, and role",
  "do_not_change": ["list of important objects/architecture that must remain unchanged"],
  "background_to_preserve": "short description of walls/floor/windows/lighting/camera that must remain stable",
  "ambiguity_note": "empty string if clear; otherwise explain ambiguity"
}}
""".strip()


def _analyze_edit_target(image: Image.Image, target: str, operation: str, edit_request: str) -> Dict[str, Any]:
    """Use Gemini vision/text to turn a parsed target into visual localization text."""
    if not GEMINI_EDIT_ANALYSIS_ENABLED:
        return {
            "target_found": True,
            "target_descriptor": f"the visible {target} in the room",
            "do_not_change": ["all other furniture", "room architecture", "camera angle", "lighting"],
            "background_to_preserve": "preserve the original room shell, floor, walls, windows, doors, ceiling, lighting, and camera angle",
            "ambiguity_note": "target analysis disabled",
        }

    try:
        raw = generate_text_from_image(_target_analysis_prompt(target, operation, edit_request), image)
        parsed = _parse_json(raw) or {}
        if not isinstance(parsed, dict):
            parsed = {}
        parsed.setdefault("target_found", True)
        parsed.setdefault("target_descriptor", f"the visible {target} in the room")
        parsed.setdefault("do_not_change", ["all other furniture", "room architecture", "camera angle", "lighting"])
        parsed.setdefault("background_to_preserve", "preserve the original room shell, floor, walls, windows, doors, ceiling, lighting, and camera angle")
        parsed.setdefault("ambiguity_note", "")
        return parsed
    except Exception as exc:  # noqa: BLE001
        return {
            "target_found": True,
            ...
            "do_not_change": ["all other furniture", "room architecture", "camera angle", "lighting"],
            "background_to_preserve": "preserve the original room shell, floor, walls, windows, doors, ceiling, lighting, and camera angle",
            "ambiguity_note": f"target analysis failed: {type(exc).__name__}: {exc}",
        }


def _preservation_contract(analysis: Dict[str, Any]) -> str:
    do_not_change = analysis.get("do_not_change") or []
    if isinstance(do_not_change, list):
        do_not_change_text = "; ".join(str(x) for x in do_not_change[:8])
    else:
        do_not_change_text = str(do_not_change)
    return (
        "Preservation contract: keep every unmentioned part of the image unchanged. "
        f"Do not change: {do_not_change_text}. "
        f"Background/camera to preserve: {analysis.get('background_to_preserve', '')}. "
        "Preserve the original camera angle, room geometry, walls, floor, ceiling, windows, doors, lighting direction, shadows, and image aspect ratio. "
        "Do not add text, labels, logos, watermarks, borders, or UI elements."
    )


def _gemini_targeted_edit(
    image: Image.Image,
    *,
    operation: str,
    target: str,
    ...
    """Gemini-native replacement for semantic-mask object editing."""
    analysis = _analyze_edit_target(image, target, operation, user_query or edit_instruction)
    descriptor = analysis.get("target_descriptor") or f"the visible {target} in the room"
    ambiguity = analysis.get("ambiguity_note") or ""

    prompt = f"""
You are performing a precise TARGETED IMAGE EDIT on the provided room image.
This is an image-editing task, not a full redesign.

Target localization:
- Edit only: {descriptor}
- Parsed target category: {target}
- Ambiguity note: {ambiguity}

Requested edit:
{edit_instruction}

{_preservation_contract(analysis)}

Critical constraints:
- If this is not a removal, keep the edited target in the same approximate position unless the request explicitly asks to move it.
- If this is a removal, remove the target completely and inpaint the background naturally.
- Do not duplicate the target object.
- Do not change unrelated furniture, decor, architecture, windows, doors, floor, ceiling, or camera perspective.
- The result should look photorealistic and internally consistent.
- Return only the edited image.
""".strip()
    return _gemini(image, prompt)


def op_recolor(image: Image.Image, target: str, color: str, user_query: Optional[str] = None, **_: Any) -> Image.Image:
    resolve_target(target)
    if target.lower() in {"rug", "carpet", "floormat"}:
        instruction = (
            f"Change only the visible {target}'s colour to a muted, tasteful {color}. "
            f"The {target} must clearly become {color}, but its shape, pattern/texture, size, position, and edges should remain stable. "
            "Objects sitting on top of it must not be recoloured or moved."
        )
    else:
        instruction = (
            f"Change only the visible {target}'s colour/upholstery to {color}. "
            f"The {target} must clearly become {color}, while preserving the same shape, size, material realism, and position."
        )
    return _gemini_targeted_edit(image, operation="recolor", target=target, edit_instruction=instruction, user_query=user_query)


def op_swap(image: Image.Image, target: str, new_object: str, user_query: Optional[str] = None, **_: Any) -> Image.Image:
    resolve_target(target)
    instruction = (
        f"Replace only the visible {target} with {new_object}. "
        "The replacement should occupy the same approximate footprint, scale, perspective, and functional role. "
        "Blend it naturally with existing lighting and shadows."
    )
    return _gemini_targeted_edit(image, operation="swap", target=target, edit_instruction=instruction, user_query=user_query)


def op_restyle_object(image: Image.Image, target: str, style: str, user_query: Optional[str] = None, **_: Any) -> Image.Image:
    resolve_target(target)
    instruction = (
        f"Restyle only the visible {target} according to: {style}. "
        "Keep its position and approximate footprint. Preserve object integrity, including legs, arms, cushions, shelves, or handles where applicable."
    )
    return _gemini_targeted_edit(image, operation="restyle_object", target=target, edit_instruction=instruction, user_query=user_query)


def op_change_material(image: Image.Image, target: str, material: str, user_query: Optional[str] = None, **_: Any) -> Image.Image:
    resolve_target(target)
    instruction = (
        f"Change only the visible {target}'s material/surface finish to {material}. "
        "Keep its exact shape, position, size, and function. The material should look photorealistic under the existing lighting."
    )
    return _gemini_targeted_edit(image, operation="change_material", target=target, edit_instruction=instruction, user_query=user_query)
...
def op_remove(image: Image.Image, target: str, use_gemini: bool = False, user_query: Optional[str] = None, **_: Any) -> Image.Image:
    resolve_target(target)
    instruction = (
        f"Remove only the visible {target} completely. Inpaint the area it occupied with plausible matching floor, wall, shadow, and background continuity. "
        "The final image should look as if the object was never there. Do not replace it with another object."
    )
    return _gemini_targeted_edit(image, operation="remove", target=target, edit_instruction=instruction, user_query=user_query)
...
OPERATIONS: Dict[str, Dict[str, Any]] = {
    "recolor": {"fn": op_recolor, "backend": "gemini", "deterministic": False},
    "swap": {"fn": op_swap, "backend": "gemini", "deterministic": False},
    "restyle_object": {"fn": op_restyle_object, "backend": "gemini", "deterministic": False},
    "change_material": {"fn": op_change_material, "backend": "gemini", "deterministic": False},
    "remove": {"fn": op_remove, "backend": "gemini", "deterministic": False},
...
def apply_operation(
    ...
    **kwargs: Any,
    ...
    # Only spatial/rearrange operations use local best-of-N selection because the
    # selection criterion is operation-specific. For all other Gemini edits,
    # apply_reliable(...) handles retry + verification; generating N candidates
    # here and returning the first wastes calls without improving quality.
    if not spec.get("spatial") or best_of_n <= 1:
...
_FALLBACK: Dict[str, Callable[..., Image.Image]] = {}  # Gemini-only build; no secondary backend fallback
...
class DesignSession:
    ...
    def generate_variants(self, brief: str, pipelines: Optional[List[str]] = None) -> Dict[str, Image.Image]:
        ...
        max_workers = max(1, min(len(names), MAX_CONCURRENT_VARIANT_PIPELINES))
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
...
OBJECT_ALIASES = {
    ...
    "cabinet": [
        "media console", "console", "cabinet", "cupboard", "storage cabinet",
        "wardrobe", "dresser", "sideboard", "credenza", "storage unit",
    ],
...
def parse_edit_request(user_query: str, product_image: Optional[Image.Image] = None) -> Optional[EditPlan]:
    ...
    if re.search(r"\b(remove|delete|get rid of|take out)\b", lower) and target:
        return EditPlan(
            ...
            kwargs={"target": target, "user_query": text},
    ...
    if target and material and re.search(r"\b(material|make|change|turn)\b", lower):
        return EditPlan(
            ...
            kwargs={"target": target, "material": material, "user_query": text},
            parser_confidence=0.8,
        )
    ...
    if target and color and re.search(r"\b(recolor|recolour|colour|color|make|turn|change)\b", lower):
        return EditPlan(
            ...
            kwargs={"target": target, "color": color, "user_query": text},
    ...
    if re.search(r"\b(replace|swap)\b", lower) and target:
        ...
        return EditPlan(
            ...
            kwargs={"target": target, "new_object": new_object, "user_query": text},
    ...
    if target and re.search(r"\b(restyle|style|make|modern|cozy|japandi|scandinavian|minimal|boho|industrial)\b", lower):
        return EditPlan(
            ...
            kwargs={"target": target, "style": text, "user_query": text},
...
def reset_runtime_clients() -> None:
    ...
    global _image_genai_client, _text_genai_client, _embedding_client, _anthropic_client
    _image_genai_client = None
    _text_genai_client = None
    _embedding_client = None
    _anthropic_client = None
...
def runtime_location_summary() -> Dict[str, Any]:
    return {
        ...
        "IMAGE_ITERATION_PROJECT_ID_env": os.getenv("IMAGE_ITERATION_PROJECT_ID"),
        ...
        "GEMINI_TEXT_LOCATION": GEMINI_TEXT_LOCATION,
        "GEMINI_IMAGE_LOCATION": GEMINI_IMAGE_LOCATION,
        "GEMINI_IMAGE_MODEL": GEMINI_IMAGE_MODEL,
        "LEGACY_IMAGEN_LOCATION": IMAGEN_LOCATION,
        "LEGACY_IMAGEN_MODEL": IMAGEN_MODEL,
        "LEGACY_IMAGEN_GEN_MODEL": IMAGEN_GEN_MODEL,
        "IMAGE_EDIT_BACKEND": GEMINI_IMAGE_MODEL,
        "GEMINI_EDIT_ANALYSIS_ENABLED": GEMINI_EDIT_ANALYSIS_ENABLED,
        "EMBED_LOCATION": EMBED_LOCATION,
        "MAX_CONCURRENT_IMAGE_GEN": MAX_CONCURRENT_IMAGE_GEN,
        "MAX_CONCURRENT_VARIANT_PIPELINES": MAX_CONCURRENT_VARIANT_PIPELINES,
        "MAX_CONCURRENT_E_BRANCHES": MAX_CONCURRENT_E_BRANCHES,
        "EMBED_MODEL": EMBED_MODEL,
        "image_object_edit_backend": GEMINI_IMAGE_MODEL,
        "legacy_imagen_disabled": True,
        "secondary_fallbacks_enabled": bool(_FALLBACK),
    }


def preflight_image_edit_backend() -> Dict[str, Any]:
    """Validate the configured Gemini image edit backend enough to catch env/import issues."""
    try:
        project_id = _require_project_id()
        return {
            "ok": True,
            "project_id": project_id,
            "backend": GEMINI_IMAGE_MODEL,
            "location": GEMINI_IMAGE_LOCATION,
            "analysis_enabled": GEMINI_EDIT_ANALYSIS_ENABLED,
            "note": "All visual edit operations route to Gemini image editing.",
        }
    except Exception as exc:  # noqa: BLE001
        return {
            "ok": False,
...
def preflight_imagen_access() -> Dict[str, Any]:
    """Backward-compatible shim for older callers. Imagen is intentionally not used."""
    out = preflight_image_edit_backend()
    out.update({
        "skipped": True,
        "legacy_imagen_disabled": True,
        "note": "Imagen preflight skipped: all image editing operations now use Gemini image editing.",
    })
    return out
...
def _tries_for_operation(op: str) -> int:
    """Operation-specific retry budget for Gemini-native image editing."""
    defaults = {
        "remove": 2,
        "recolor": 2,
        "swap": 2,
    ...
    override = os.getenv(f"RELIABLE_IMAGE_EDIT_TRIES_{op.upper()}")
    if override:
        try:
            return max(1, int(override))
        except ValueError:
            pass
    return max(1, int(os.getenv("RELIABLE_IMAGE_EDIT_TRIES", str(defaults.get(op, 1)))))
...
def run_reliable_edit(
    ...
    try:
        result = apply_reliable(
            ...
            plan.op,
            goal=plan.goal,
            tries=_tries_for_operation(plan.op),
